#Use this for MultiDimentional Array
	...
	for item in t[2].children:
		if len(item.children)==0:
			...
			if item.type=="array":
				...
				t[0].addCode(item.code)
				size = 0
				size = len(item.children[1].children)
...
def p_expression_15(t):
	...
	t[0].dataType = res[0]
	gen = ["aload "+str(res[2])]+t[0].code
	t[0].code = gen
	dim = []
	totalSize = 1
	for i in res[1].children:
		dim.append(int(i.type))
		totalSize=totalSize*int(i.type)
	p = 1
	size = len(t[1].children[1].children)
	for i in range(size-1):
		p *= dim[size-i-1]
		t[0].addCode(["istore 20"])
		t[0].addCode(["ldc "+str(p)])
		t[0].addCode(["imul"])
		t[0].addCode(["iload 20"])
		t[0].addCode(["iadd"])
...
def p_assignment_12(t):
	...
	t[0].addCode([javaType(res[0]).lower()+"aload"])
	t[0].addCode(t[3].code)
	t[0].addCode([javaType(res[0]).lower()+"rem"])
	t[0].addCode([javaType(res[0]).lower()+"astore"])
	t[0].addCode(["iconst_1"])
